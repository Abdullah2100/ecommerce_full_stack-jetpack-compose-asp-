...
internal class TimePickerViewModel : ViewModel() {
    ...
    private val _uiState: MutableStateFlow<TimePickerUiState> = MutableStateFlow(TimePickerUiState())
    val uiState: LiveData<TimePickerUiState> = _uiState.asLiveData()
    ...
    fun updateSelectedMinuteIndex(index: Int) {
        viewModelScope.launch {
            _uiState.emit( _uiState.value.copy(selectedMinuteIndex = index))
        }
    }
    ...
    fun updateSelectedTimeOfDayIndex(index: Int) {
        if (index != _uiState.value.selectedTimeOfDayIndex) {
            viewModelScope.launch {
                _uiState.emit( _uiState.value.copy(selectedTimeOfDayIndex = index))
